...
namespace example
{
    ...
    bool Sprite::intersects (const Sprite & other, float scale)
    {
        // Se determinan las coordenadas de la esquina inferior izquierda y de la superior derecha
        // de este sprite:

        float this_left    = 40+this->get_left_x   () /scale ;
        float this_bottom  = 40+this->get_bottom_y () /scale;
        float this_right   = -40+this_left   + this->size.width;
        float this_top     = -40+this_bottom + this->size.height ;

        // Se determinan las coordenadas de la esquina inferior izquierda y de la superior derecha
        // del otro sprite:

        float other_left   = 40+other.get_left_x   () /scale ;
        float other_bottom = 40+other.get_bottom_y () /scale;
        float other_right  = -40+other_left  + other.size.width;
        float other_top    = -40+other_bottom + other.size.height;
    ...
    bool Sprite::contains(const Point2f & point, float scale){
        float this_left = this->get_left_x ()/scale;

        if (point.coordinates.x () > this_left)
        {
            float this_bottom = this->get_bottom_y ()/scale;
